...
func update_highscores_table():
	...
	request.request(HIGHSCORE_SINGLETON.BACK_END_URL + HIGHSCORE_SINGLETON.GAME_NAME)
	#request.queue_free()
...
func check_if_players_score_is_high_enough():
	...
	request.request(HIGHSCORE_SINGLETON.BACK_END_URL + HIGHSCORE_SINGLETON.GAME_NAME + 
			"?score=" + str(HIGHSCORE_SINGLETON.SCORE))
	#request.queue_free()
...
func post_highscores_online():
	var url = HIGHSCORE_SINGLETON.BACK_END_URL + HIGHSCORE_SINGLETON.GAME_NAME
	...
	request.request(url, headers, false, HTTPClient.METHOD_POST, payload_string)
	HIGHSCORE_SINGLETON.SCORE = null
	#request.queue_free()


func _on_post_highscores_request_completed(_result, _response_code, _headers, body):
	update_highscores_table()
